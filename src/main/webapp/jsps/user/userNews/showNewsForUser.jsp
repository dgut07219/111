...
<body>
	...
	<center>
		<a
			href="<c:url value="/NewsServlet?pc=1&method=findAllNews&name=${newsType}&type=user"/>">首页</a>
		<c:if test="${pb.pc > 1}">
			<a
				href="<c:url value="/NewsServlet?pc=${pb.pc-1}&method=findAllNews&name=${newsType}&type=user"/>">上一页</a>
		</c:if>
		...
		<c:forEach var="i" begin="${begin}" end="${end}">
		  ...
		  [${i}]
				...
				<c:otherwise>
					<a
						href="<c:url value="/NewsServlet?pc=${i}&method=findAllNews&name=${newsType}&type=user"/>">[${i}]
					</a>
				</c:otherwise>
			</c:choose>
		</c:forEach>
		...
		<c:if test="${pb.pc < pb.tp}">
			<a
				href="<c:url value="/NewsServlet?pc=${pb.pc+1}&method=findAllNews&name=${newsType}&type=user"/>">下一页</a>
		</c:if>
		<a
			href="<c:url value="/NewsServlet?pc=${pb.tp}&method=findAllNews&name=${newsType}&type=user"/>">尾页</a>
	</center>
	<div align="center">
